...
public class Jenabean {
	...
	// START EDIT wvw
	public static <E> E loadDeep(Class<E> c, String id) throws NotFoundException {
		return myself.reader().load(c, id, false);
	}
	// END EDIT wvw
	...
	public static <T> Collection<T> query(Class<T> c, String query) {
		return query(c, query, myself.model);
	}
	
	// START EDIT wvw
	public static <T> Collection<T> query(Class<T> c, String query, Model model) {
		return Sparql.exec(model, c, query);
	}
	// END EDIT wvw
