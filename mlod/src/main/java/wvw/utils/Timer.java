...
public class Timer {
	...
	public static void newRun() {
		if (times == null || !times.isEmpty()) {
			times = new HashMap<String, TimeEntry>();
	...
	public static void end(String tag) {
		...
		double val = ((double) entry.getTime() / 1000000000);
		Log.i("END " + tag + " (time = " + val + " s)\n");
	}
	...
	public static void printTimes(TimeUnits unit) {
		if (!stack.isEmpty())
			Log.e("ERROR: Timer.stack not empty:\n" + stack);

		// Log.i(times);
		times.entrySet().stream().sorted((e1, e2) -> e1.getValue().compareTo(e2.getValue())).forEach(e -> {
			double val = -1;
			switch (unit) {

			case SEC:
				val = ((double) e.getValue().getTime()) / 1000000000;
				break;

			case MS:
				val = ((double) e.getValue().getTime()) / 1000000;
				break;
			}

			Log.i(e.getKey() + ": " + val + " " + unit);
		});
	}
	...
	private static Map<String, TimeEntry> getTotals() {
		Map<String, TimeEntry> totals = new HashMap<String, TimeEntry>();
	...
	private static class TimeEntry implements Comparable<TimeEntry> {
		...
		@Override
		public int compareTo(TimeEntry e) {
			return Long.valueOf(start).compareTo(e.start);
		}

		public String toString() {
			return "{ " + baseTag + ": @" + start + " - time = " + time + " - count = " + cnt + " }";
		}
	}
}
